...
//				VectorUtil.fastCanSeeTwo( player.getEyeLocation(), location );
				...
				if ( args[ 0 ].equalsIgnoreCase( "form" ) ) {
					...
					GunsmokeStructure structure = null;
					int blocks = 0;
					int hp = 0;
					for ( int x = -2; x < 3; x++ ) {
						for ( int y = -2; y < 3; y++ ) {
							for ( int z = -2; z < 3; z++ ) {
								...
								Material bType = newLoc.getBlock().getType();
								if ( bType != Material.AIR ) {
									hp += blockManager.getDefaultResistanceFor( bType ) + 10;
									if ( structure == null ) {
										structure = new GunsmokeStructure( newLoc, 0 );
									}
									structure.addBlock( new GunsmokeBlock( newLoc, 0 ) );
									blocks++;
									
									GunsmokeBlock oldBlock = blockManager.getBlockAt( newLoc );
									if ( oldBlock != null ) {
										blockManager.unregisterBlock( oldBlock );
									}
								}
							}
						}
					...
					if ( blocks > 0 ) {
						player.sendMessage( "A new structure was formed with " + blocks + " blocks!" );
						structure.setMaxHealth( hp );
						structure.setHealth( hp );
